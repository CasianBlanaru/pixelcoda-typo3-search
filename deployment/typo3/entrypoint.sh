...
if [ "$db_configured" = true ]; then
    ...
    # Always create/reset pixelcoda admin user with fresh hash
    echo "Creating pixelcoda admin user..."
    admin_hash="$(php -r 'echo password_hash("Pixelcoda123!", PASSWORD_ARGON2ID, ["memory_cost"=>65536,"time_cost"=>4,"threads"=>1]);')"
    mysql \
        --host="${TYPO3_DB_HOST}" \
        --port="${TYPO3_DB_PORT}" \
        --user="${TYPO3_DB_USERNAME}" \
        --password="${TYPO3_DB_PASSWORD}" \
        "${TYPO3_DB_DBNAME}" \
        -e "DELETE FROM be_users WHERE username='pixelcoda'; INSERT INTO be_users (pid,tstamp,crdate,username,password,admin,disable,deleted,realName,email,lang) VALUES (0,UNIX_TIMESTAMP(),UNIX_TIMESTAMP(),'pixelcoda','${admin_hash}',1,0,0,'PixelCoda Admin','admin@pixelcoda.de','de');" \
        && echo "pixelcoda admin created" || echo "Admin user SQL failed"
